<div class="modal-content">
    ...
    <div class="modal-footer">
        ...
        <button type="submit" name="saveEditFreezeDepartment" id="saveEditFreezeDepartment"
            class="btn btn-warning btn-form-block-overlay"><i class='menu-icon tf-icons bx bxs-save'></i>
            บันทึกข้อมูล</button>
    </div>

    <script type="text/javascript"
        src="{{ asset('/assets/custom/freezeAccount/department/func_edit.js?v=') }}@php echo date("H:i:s") @endphp"></script>
    <script src="{{ asset('assets/js/forms-extras.js') }}"></script>
    <script>
        // Initialize Select2 when the modal is shown
        $('#editFreezeAccountDepartmentModal').on('shown.bs.modal', function() {
            $('#bank_id').select2({
                placeholder: 'เลือกข้อมูล',
                allowClear: true, // Ensures the clear option is available
                dropdownParent: $(
                    '#editFreezeAccountDepartmentModal'), // Ensures dropdown appears within the modal
